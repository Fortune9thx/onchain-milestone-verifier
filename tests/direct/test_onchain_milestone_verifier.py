...
def test_verify_milestone_satisfied_releases_funds(direct_deploy, direct_vm):
    ...
    direct_vm.mock_llm(r".", 'Result: {"outcome": "SATISFIED"}')
...
def test_verify_milestone_satisfied_only_via_exact_enum_value(direct_deploy, direct_vm):
    ...
    _mock_state_read(direct_vm, "DONE")
    direct_vm.mock_llm(r".", 'Result: {"outcome": "PROBABLY_SATISFIED"}')
...
def test_verify_milestone_not_satisfied_no_release(direct_deploy, direct_vm):
    ...
    direct_vm.mock_llm(r".", 'Result: {"outcome": "NOT_SATISFIED"}')
...
def test_verify_milestone_marker_detects_change_past_truncation_cutoff(
    direct_deploy, direct_vm
):
    """Regression test for a real GenLayer Portal steward finding: the
    unchanged-state marker must be computed from the COMPLETE observed
    state, not the display/prompt-truncated one. Two states here are
    identical for the first MAX_OBSERVED_STATE_CHARS characters and
    differ only after that cutoff -- a milestone-relevant change hiding
    exactly there must still be detected as a genuine change, not
    silently treated as unchanged forever."""
    contract = direct_deploy(CONTRACT_PATH)
    mod = _module()
    warp_with_message(direct_vm, "2026-01-01T00:00:00Z")
    _, tranche_id = _setup_program_and_tranche(contract, direct_vm)

    long_prefix = "X" * mod.MAX_OBSERVED_STATE_CHARS
    state_a = long_prefix + "-END-A"
    state_b = long_prefix + "-END-B"

    _mock_state_read(direct_vm, state_a)
    direct_vm.mock_llm(r".", 'Result: {"outcome": "NOT_SATISFIED"}')
    contract.verify_milestone(tranche_id)

    warp_with_message(direct_vm, "2026-01-02T00:01:00Z")  # past the cooldown
    direct_vm.clear_mocks()
    _mock_state_read(direct_vm, state_b)
    direct_vm.mock_llm(r".", 'Result: {"outcome": "SATISFIED"}')

    # Must NOT be rejected as "state has not changed" -- the full state
    # genuinely differs, even though the truncated prefix shown to the
    # LLM does not.
    verification_id = contract.verify_milestone(tranche_id)
    verification = json.loads(contract.get_verification(verification_id))
    assert verification["outcome"] == "SATISFIED"
...
def test_verify_milestone_insufficient_state_via_llm_judgment(direct_deploy, direct_vm):
    ...
    direct_vm.mock_llm(r".", 'Result: {"outcome": "INSUFFICIENT_STATE"}')
