...
**Deployed on GenLayer Studio Devnet (current, v1.5.0):** [`0x6892dD5Cdccaa536aA86ba38d14cB99F2FFE50AA`](https://explorer-studio-dev.genlayer.com/address/0x6892dD5Cdccaa536aA86ba38d14cB99F2FFE50AA). This release migrates off Bradbury following a confirmed, multi-day network outage there (see [docs/DESIGN.md §16](docs/DESIGN.md#16-the-bradbury-outage-the-migration-to-studio-devnet-and-every-api-surface-change-it-required)), porting the contract to the GenLayer Consensus v0.6 / SDK v0.3.0 API with no public method signature changes. It still carries the marker-truncation fix from a real GenLayer Portal steward finding — see [docs/DESIGN.md §15](docs/DESIGN.md#15-a-third-real-steward-finding-the-anti-farming-marker-was-hashed-from-truncated-state-not-complete-state) and `CHANGELOG.md`'s `[1.4.0]`/`[1.5.0]` entries. Live on Studio Devnet with real GEN: `create_program` (2 GEN escrowed) and `register_tranche` (1 GEN tranche against [`DeploymentStatusTarget`](examples/deployment_status_target.py) at `0xfBECDFaB8671f009D22E65ea90e49512F6EC2eFE`) both succeeded, and `withdraw_unallocated` finalized cleanly (`AGREE`, 5/5 validators). `verify_milestone`'s live run is currently blocked by a confirmed, GenLayer-team-acknowledged Studio Devnet infrastructure gap (specific validator nodes configured with an unreachable LLM provider), documented transparently in §16 rather than hidden — the underlying mechanism was already live-verified end-to-end with real GEN and a real fund release on the original Bradbury 1.0.0 deployment; see `CHANGELOG.md`'s `[1.0.0]` entry for that record.
