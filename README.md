...
**Deployed on GenLayer Studio Devnet (current, v1.6.0):** [`0x73bB5342Cd0AF3BB52cEBc300a3F360fffd66Bb0`](https://explorer-studio-dev.genlayer.com/address/0x73bB5342Cd0AF3BB52cEBc300a3F360fffd66Bb0). This release migrates off Bradbury following a confirmed, multi-day network outage there and ports the contract to the GenLayer Consensus v0.6 / SDK v0.3.0 API (see [docs/DESIGN.md §16](docs/DESIGN.md#16-the-bradbury-outage-the-migration-to-studio-devnet-and-every-api-surface-change-it-required)), with no public method signature changes. It carries the marker-truncation fix from a real GenLayer Portal steward finding ([docs/DESIGN.md §15](docs/DESIGN.md#15-a-third-real-steward-finding-the-anti-farming-marker-was-hashed-from-truncated-state-not-complete-state)) plus a disclosed, narrow trade-off found via a live controlled A/B test: `verify_milestone`'s cross-contract read now requests `LATEST_DECIDED` state rather than the architecturally-preferred `LATEST_FINALIZED`, working around a confirmed Studio Devnet `CallContract`-dispatch bug that otherwise hangs the call indefinitely — see §16 for the full repro and the exact trade-off accepted. **Live on Studio Devnet with real GEN, full lifecycle:** `create_program`, `register_tranche`, and `verify_milestone` all completed live, both outcomes proven -- `NOT_SATISFIED` against the target's `"PENDING"` state (zero funds moved) and, after the target's `mark_live()`, `SATISFIED` with a real 1 GEN release, tranche flipped to `RELEASED`. `withdraw_unallocated` also separately verified live (`AGREE`, 5/5 validators). 41/41 tests passing, `genvm-lint check`/`validate` both clean.
...
## Why re-verification requires genuine state change, and is cooldown-limited, not count-limited

`verify_milestone`'s judgment is a real LLM call, and `gl.eq_principle.strict_eq` only guarantees that *whatever* a given round's independent judgments agree on is accepted — it does not guarantee two separate rounds over the identical, unchanged observed state will agree with *each other*. Left ungated, a party to a tranche could re-trigger verification against the same unchanged target state indefinitely, for gas alone, until one round's independent judgments happened to land on an unwarranted `SATISFIED` — a small, bounded cost against the full tranche amount as payoff. Two gates close this: (1) each attempt's observed-state hash must differ from the tranche's last attempt, checked and enforced *before* any cross-contract read result is used for anything else, so a same-state retry costs only the deterministic read; (2) consecutive attempts must be spaced at least `MIN_VERIFICATION_INTERVAL_SECONDS` apart, checked even before that.

An earlier version of gate (2) used a fixed attempt count instead of a cooldown, and that count turned out to be its own vulnerability: a bad-faith funder could exhaust it on genuinely honest, early, not-yet-satisfied readings before the grantee ever got a chance to prove real completion, then reclaim the funds once the count ran out and the tranche could never be checked again. A cooldown has no such ceiling — however many honest check-ins already happened, a tranche can always eventually get one more, correct one once real progress occurs. See [docs/DESIGN.md §13](docs/DESIGN.md#13-a-third-review-round-two-of-12s-own-fixes-were-themselves-findings) for the full account.

Gate (1)'s marker is hashed from the **complete** observed state, not the truncated string used for the LLM prompt and audit storage — an earlier version hashed the truncated one, so two genuinely different states sharing an identical first `MAX_OBSERVED_STATE_CHARS` characters produced the same marker, making a milestone-relevant change past that cutoff permanently invisible to this gate. A real GenLayer Portal steward review caught this; see [docs/DESIGN.md §15](docs/DESIGN.md#15-a-third-real-steward-finding-the-anti-farming-marker-was-hashed-from-truncated-state-not-complete-state).

## Why there is no fund-transfer retry mechanism

An earlier revision added `retry_release`, a reconciliation tool for a `RELEASED` tranche whose `emit_transfer` might not have arrived (GenVM gives contract code no delivery confirmation), first uncapped, then capped at a small fixed count. A real GenLayer Portal steward review correctly identified that a cap doesn't make a retry *safe*, only *bounded*: nothing on-chain can tell "the first transfer genuinely failed" apart from "it succeeded and someone is retrying anyway," so even a single permitted retry is a guaranteed, unconditional duplicate payment whenever the original transfer actually succeeded — and since this contract pools every program's GEN in one shared balance, that duplicate payment can draw on funds belonging to an entirely different program. There is no information GenVM exposes to contract code that could gate a retry safely, so the method is removed entirely rather than redesigned. This returns the contract to the same accepted trade-off used by every other payout-bearing contract in this account's history: a failed async transfer is not on-chain-recoverable, a documented platform limitation, not a defect this contract tries to paper over. See [docs/DESIGN.md §14](docs/DESIGN.md#14-a-real-genlayer-portal-steward-review-found-what-three-internal-review-rounds-missed) for the full account.
...
41 direct-mode tests covering the full program → tranche → verify → release lifecycle, fund-accounting correctness (escrow, allocation, release, unallocated withdrawal, stale-tranche reclaim with CEI ordering throughout), authorization on every write method, input validation, the deterministic-read-failure short-circuit (verified to skip the LLM call entirely), the anti-farming gates (unchanged-state rejection, the verification cooldown, and the change-marker's correctness past the truncation cutoff), and the pure helper functions in isolation. `genvm-lint check`/`validate` both pass with zero warnings. Because this contract's core mechanism (dynamic cross-contract view calls) has no built-in mock in `gltest`'s direct mode, a project-specific mock (`tests/direct/conftest.py::install_call_contract_mock`) was built and validated with a standalone probe *before* the real contract was written — see [docs/DESIGN.md](docs/DESIGN.md) for that process, including a real bug the test suite caught mid-build, and §§14–15 for two real steward-caught bugs the test suite did *not* catch until after the design itself was corrected.
