...
**Deployed on Testnet Bradbury (current, v1.2.0):** [`0x5b20f2833D1BCad3830eb08C40559AA57B0a7D0f`](https://explorer-bradbury.genlayer.com/address/0x5b20f2833D1BCad3830eb08C40559AA57B0a7D0f) — deploy tx `0xb09cd5aacae84b197b26eb8ecdf706c1cd1ea26f30e048cd404a0ef78a3dd451`, confirmed `FINALIZED`/`AGREE`/`FINISHED_WITH_RETURN` (5/5 validators agreed), post-deploy read confirmed (`get_program_count() == 0`). This redeploy carries the fixes from a third adversarial review pass documented in [docs/DESIGN.md §13](docs/DESIGN.md#13-a-third-review-round-two-of-12s-own-fixes-were-themselves-findings) and `CHANGELOG.md`'s `[1.2.0]` entry. The mechanism itself (deterministic cross-contract read + independent judgment + fund release) was already live-verified end-to-end with real GEN on the original 1.0.0 deployment; see `CHANGELOG.md`'s `[1.0.0]` entry for that record.
...
## Why `retry_release` is capped

Every other fund-moving method in this contract changes some piece of state specifically to block itself from being re-triggered: `verify_milestone`'s `SATISFIED` branch flips the tranche to `RELEASED` before transferring; `withdraw_unallocated` decrements the program's escrow until nothing is left. `retry_release` — a deliberate escape hatch for a transfer that may not have arrived, since GenVM gives contract code no delivery confirmation — is the one fund-moving method with no natural state change to lean on, and this contract holds one pooled GEN balance shared across every program, not a segregated balance per tranche. Left uncapped, it would have been a standing, repeatable drain on every program's escrow rather than a narrow reconciliation tool. It is capped at `MAX_RELEASE_RETRIES` (1), tracked via `release_retry_count` on the tranche, written to storage before the transfer is attempted.
...
| `retry_release(tranche_id) -> None` | write | Funder-or-grantee. Manual reconciliation escape hatch: re-issues the transfer for an already-`RELEASED` tranche whose original `emit_transfer` may not have arrived. Capped at `MAX_RELEASE_RETRIES` total |
